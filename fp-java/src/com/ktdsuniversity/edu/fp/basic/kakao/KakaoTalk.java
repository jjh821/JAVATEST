...
public class KakaoTalk {
	...
	public static void main(String[] args) {
		...
		fl.add( new Friend("김학철", "2001-02-01") );
		fl.add( new Friend("이병옥", "1999-02-01") );
...
//				&&	birthDate.isAfter(LocalDate.now().minusDays(8)));
		...
		LocalDate now = LocalDate.now();
		LocalDate birthdate = friend.getBirthdate().withYear( now.getYear());
			return birthdate.isBefore(now) && birthdate.isAfter(now.minusDays(8));
		});
		...
		System.out.println("이름이  '이'로 시작하면서 나이가 20세 이상인 친구목록");
		fl.printSpecialFriends(friend -> 
			friend.getName().startsWith("이") && Period.between(friend.getBirthdate(), LocalDate.now()).getYears() >= 20
			
		);
		System.out.println("전체친구 목록");
