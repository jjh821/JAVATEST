...
public class Buyer {
	...
	private int itemCount;  //상품 구매 개수
	private int wallet;
	...
	public void buyFrom(Seller seller, int productCount) {
		...
		// 5. 구매자: (내가 5천원이 있나..?)
			if(this.isEnoughMoney(price * productCount)) {
				...
				//			들 수 있다!
				// 9. 구매자: 5개만 주시겠어요?
				//10. 판매자: 두쫀쿠 5개를 포장해서 구매자에게 준다(판매자의 재고가 감소);
				int stock	= seller.getStock();
				stock -= productCount;
				seller.setStock(stock);
				//11. 구매자: 여기 5천원 드릴게요. (구매자의 돈이 감소, 판매자의 자산이 증가, 구매자의 무게와 구매개수 증가)
				this.wallet -= price * productCount;
				int acount = seller.getAccount();
				acount += price * productCount;
				seller.setAccount(acount);
